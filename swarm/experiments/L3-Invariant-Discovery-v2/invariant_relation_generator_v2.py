...
def extract_structural_features(
    ...
    features: Dict[str, Any] = {
        ...
        "left_is_prefix_of_result": (
            result.startswith(left) if left else False
        ),
        "right_is_suffix_of_result": (
            result.endswith(right) if right else False
        ),
        "right_is_prefix_of_result": (
            result.startswith(right) if right else False
        ),
        "left_is_suffix_of_result": (
            result.endswith(left) if left else False
        ),
    ...
    features["left_right_symbol_set_equal"] = (
        set(left) == set(right)
    )
    features["left_result_symbol_set_equal"] = (
        set(left) == set(result)
    )
    features["right_result_symbol_set_equal"] = (
        set(right) == set(result)
    )

    features["left_right_multiset_equal"] = (
        Counter(left) == Counter(right)
    )
    features["left_result_multiset_equal"] = (
        Counter(left) == Counter(result)
    )
    features["right_result_multiset_equal"] = (
        Counter(right) == Counter(result)
    )
...
def _feature_value_key(value: Any) -> str:
    ...
    if isinstance(value, (list, tuple, set)):
        return repr(
            tuple(_feature_value_key(item) for item in value)
        )

    return repr(value)


def _candidate_relation_name(
    feature_name: str,
    feature_value: Any,
) -> str:
    return (
...
def generate_pairwise_relations(
    ...
    relation_specs = (
        ...
        (
            "left_unique_symbol_count",
            "right_unique_symbol_count",
        ),
        (
            "left_unique_symbol_count",
            "result_unique_symbol_count",
        ),
        (
            "right_unique_symbol_count",
            "result_unique_symbol_count",
...
def build_relation_inventory(
    ...
    observation_relations = generate_observation_relations(
        observations
    )
    cross_record_relations = generate_cross_record_relations(
        observations
    )
    pairwise_relations = generate_pairwise_relations(
        observations
    )
